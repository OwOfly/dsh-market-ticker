...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 0.42 -> +0.42%; flat renders an em dash. */
...
.mt-statusbar-group{align-items:center;gap:12px;display:inline-flex}
...
.mt-mask{position:fixed;inset:0;z-index:1100000;background:rgba(0,0,0,.32);display:flex;align-items:center;justify-content:center;animation:mtFade .15s ease-out}
@keyframes mtFade{from{opacity:0}to{opacity:1}}
@media (prefers-reduced-motion:reduce){.mt-mask{animation:none}}
.mt-panel{width:520px;max-width:calc(100vw - 40px);max-height:min(84vh,720px);overflow-y:auto;box-sizing:border-box;background:var(--mt-panel-bg);color:var(--mt-text);border:1px solid var(--mt-border);border-radius:12px;box-shadow:0 24px 64px rgba(0,0,0,.28);padding:18px 20px;display:flex;flex-direction:column;gap:12px}
.mt-panel h2{margin:0;font-size:14px;font-weight:700;letter-spacing:.02em}
.mt-fieldset{border:none;margin:0;padding:0;display:flex;flex-direction:column;gap:6px}
.mt-fieldset legend,.mt-label{font-size:12px;font-weight:600;color:var(--mt-dim)}
.mt-row{display:flex;flex-wrap:wrap;gap:14px;row-gap:8px}
.mt-check{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;color:var(--mt-text);cursor:pointer}
.mt-check input{accent-color:var(--mt-brand);cursor:pointer}
.mt-input,.mt-textarea{box-sizing:border-box;border:1px solid var(--mt-border);background:var(--mt-input-bg);color:var(--mt-text);border-radius:8px;font:500 12.5px/1.5 var(--mt-mono);padding:7px 10px;outline:none;width:100%}
.mt-textarea{min-height:88px;resize:vertical}
.mt-input:focus,.mt-textarea:focus{border-color:var(--mt-brand)}
.mt-num{width:90px !important}
.mt-hint{font-size:11px;line-height:1.6;color:var(--mt-dim)}
.mt-hint code{font-family:var(--mt-mono);background:rgba(127,142,160,.15);border-radius:4px;padding:0 4px}
.mt-actions{display:flex;gap:10px;justify-content:flex-end;border-top:1px solid var(--mt-border);padding-top:12px}
.mt-action{border:1px solid var(--mt-border);background:transparent;color:var(--mt-text);border-radius:8px;font:600 12.5px/1 var(--mt-font);padding:9px 16px;cursor:pointer}
.mt-action:hover{background:rgba(127,142,160,.15)}
.mt-action.primary{background:var(--mt-brand);border-color:var(--mt-brand);color:#fff}
.mt-action.primary:hover{filter:brightness(1.06)}
`;
		...
		function apply(_ctx) {
			try {
				...
				function applyChrome() {
					...
					// 页面让位补偿：顶部 = 标题栏 34+1 + 跑马灯 30+1；底部 = 状态栏 26+1。
					const padTop = (settings.showTitlebar ? 35 : 0) + (settings.showTape ? 31 : 0);
					body.style.paddingTop = padTop > 0 ? `${padTop}px` : '';
					body.style.paddingBottom = settings.showStatusbar ? '27px' : '';
				...
				/** 按当前 refreshSec 重建调度器（单 timer 多 cadence：行情 + 时段）。 */
				...
				openHandlers.push(() => {
					...
					h2.textContent = '行情条设置';

					// 区域显隐
					const fsVisibility = document.createElement('fieldset');
					fsVisibility.className = 'mt-fieldset';
					const legV = document.createElement('legend');
					legV.textContent = '显示区域';
					const rowV = document.createElement('div');
					rowV.className = 'mt-row';
					const checks = {};
					...
					lbSec.textContent = '刷新间隔（秒，5~600）';
					...
					lbSec.append(inSec);
					rowNum.append(lbTitle, lbChip, lbSec);
					fsNums.append(legN, rowNum);

					// 自选列表 ×2
					const fsLists = document.createElement('fieldset');
					fsLists.className = 'mt-fieldset';
					const legL = document.createElement('legend');
					legL.textContent = '自选标的（每行一个，逗号或空格分隔）';
					const lbTape = document.createElement('label');
					lbTape.className = 'mt-label';
					lbTape.textContent = `跑马灯自选（当前 ${settings.tapeSymbols.length} 个）`;
					const taTape = document.createElement('textarea');
					taTape.className = 'mt-textarea';
					taTape.value = settings.tapeSymbols.join('\n');
					lbTape.append(taTape);
					const lbIdx = document.createElement('label');
					lbIdx.className = 'mt-label';
					lbIdx.textContent = `状态栏指数（当前 ${settings.indexSymbols.length} 个）`;
					const taIdx = document.createElement('textarea');
					taIdx.className = 'mt-textarea';
					taIdx.value = settings.indexSymbols.join('\n');
					lbIdx.append(taIdx);
					fsLists.append(legL, lbTape, lbIdx);
					...
					panel.append(h2, fsVisibility, fsNums, fsLists, hint, actions);
					mask.append(panel);
